...
export function PipelineBoard({
  ...
  return (
    ...
    >
      ...
      <div className="pipeline-scroll flex min-h-0 flex-1 snap-x snap-mandatory items-stretch gap-3 overflow-x-auto pb-3 lg:snap-none">
...
function StageColumn({
  ...
  return (
    ...
    // snap-start lands each column cleanly when swiping. On desktop the
    // columns keep a fixed width and the board scrolls horizontally.
    // The droppable ref is on the inner list region below — intentionally
    // NOT here, so a drag over the column header doesn't highlight it.
    <div className="flex w-[85vw] max-w-[300px] min-w-[260px] shrink-0 snap-start flex-col rounded-lg border border-border bg-card-2 lg:w-[280px] lg:snap-none">
      ...
      <div
        ref={setNodeRef}
        className={`mx-2 flex flex-1 flex-col gap-2 rounded-md pb-1 transition-colors ${
          isOver ? "bg-primary-soft ring-1 ring-primary/30" : ""
        }`}
      >
        {deals.length === 0 ? (
          <div className="flex h-20 items-center justify-center rounded-md border border-dashed border-border text-xs text-subtle-foreground">
            {t("dropDealHere")}
          </div>
        ) : (
          deals.map((deal) => (
      ...
      <button
        ...
        className="m-2 mt-1 flex h-8 cursor-pointer items-center gap-1.5 rounded-md px-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
      >
